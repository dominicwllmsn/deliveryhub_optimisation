...
// READ IN FILE
  ...
  cout << "Min/Max Pop: " << minPop << ", " << maxPop  << endl;
...
// OPTIMIZATION
  ...
  int N = 10; // number of iterations of for-loop
  ...
  for (int m = 0; m < 100; m++) {
    double distance = greatCircle(y, x, latitude[m], longitude[m]);
    value += distance*(population[m] - minPop)/(maxPop - minPop);
  }
  //cout << "Coords: (" << y << " ," << x <<") Initial sum: " << value << endl;
  ...
  do {
    ...
    // so maybe you want to miss that one and save some function evaluations
      ...
      for (int m = 0; m < 100; m++) {
        double distance = greatCircle(y, x, latitude[m], longitude[m]);
        newValue += distance*(population[m] - minPop)/(maxPop - minPop);
      }
    ...
    y += step * dy;
    value = minValue;
    //cout << "Coords: (" << y << " ," << x <<") Subsequent sum: " << value << endl;
